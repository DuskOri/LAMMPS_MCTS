...
- 前端顶部新增“终止”按钮，可停止当前 LAMMPS 子进程并取消剩余 MCTS 搜索轮次。
- 后台新增 `/api/stop`，区分“正在终止”和“已终止”状态，并在停止后恢复运行按钮。
- 前端 API 默认跟随当前页面的主机和端口，换端口或部署到其他设备时不再错误回连 `localhost:8010`。
- 将 LAMMPS 实时输出从运行进度卡中移出，改为主设置区下方的独立横向终端窗口。
- 搜索设置与运行进度两张主卡改为同一行等高布局，避免终端输出撑高单侧卡片。
- 修正压缩结束后前端缺少持续进度提示的问题，NEMD 稳态建立和正式采样现在分别显示当前步数。
- 运行状态接口保留最近 200 行 LAMMPS 关键输出，前端进度区新增实时命令行窗口。
- 确认密度平台、冷却和平衡完成后直接进入 NEMD；当前算例最终平衡密度为约 `0.974 g/cm3`。
