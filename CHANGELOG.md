...
- Green-Kubo 前增加高温熔融、保守预压缩、NPT 密度平台判断、冷却和最终平衡流程。
- 前端增加密度平台开关、预压缩密度、平台阈值和最大窗口数配置。
- 密度平衡不再假定所有聚合物具有统一目标密度，最终密度由 NPT 平衡自动确定。
- 前端运行流程新增独立的“体系压缩”步骤，并根据 LAMMPS 密度日志实时切换到 Green-Kubo 阶段。
- 修正 Packmol 21.2.3 不支持 `-i` 参数导致候选在装箱阶段全部失败的问题，统一改用标准输入调用。
